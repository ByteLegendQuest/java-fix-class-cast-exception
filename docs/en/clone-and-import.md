...
`git clone https://github.com/ByteLegendQuest/java-fix-class-cast-exception`

A project directory named `java-fix-class-cast-exception` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-class-cast-exception` directory which is created just now.
